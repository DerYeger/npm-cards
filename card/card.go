...
func graph(card lib.Card) {
  ...
  path := "M" + fmt.Sprint(card.Padding) + "," + fmt.Sprint(getPoint(card.Size, card.Padding, card.PackageData.WeeklyDownloads[0].Downloads, downloadsMax))
  for i := 1; i < len(card.PackageData.WeeklyDownloads); i++ {
    ...
    path = path + "L" + fmt.Sprint(xCord)  + "," + fmt.Sprint(getPoint(card.Size, card.Padding, card.PackageData.WeeklyDownloads[i].Downloads, downloadsMax))
  }
  log.Print(path)
